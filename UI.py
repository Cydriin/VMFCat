...
orig_addr_options.add_argument(
    "--orig-urn",
...
recp_addr_options.add_argument(
    "--rcpt-unitnames",
    nargs="+",
...
info_addr_options.add_argument(
    "--info-units",
    dest="info_unitnames",
    metavar="UNITNAMES",
    action="store",
...
msg_handling_options.add_argument(
    "--msg-version",
        ...
        while (cmd.lower() != VmfShell.CMD_QUIT):
            try:
                ...
                if (cmd.lower() == VmfShell.CMD_QUIT):
                    ...
                    if (len(tokens) == 1):
                        ...
                        self.logger.print_info(
                            "{:s} <file>".format(
                                VmfShell.CMD_LOAD))
                        self.logger.print_info(
                            "{:s}".format(VmfShell.CMD_QUIT))
                    else:
                        param = tokens[1]
                        if (param in list(Params.__dict__.keys())):
                            help_msg = Params.parameters[param]['help']
                            self.logger.print_info(help_msg)
                            if (len(Params.parameters[param]['choices']) > 0):
                                choices_msg = ', '.join(
                                    [choice for choice in Params.parameters[param]['choices']])
                                self.logger.print_info(
                                    "Available values: {:s}".format(choices_msg))
                        else:
                    ...
                    if (len(tokens) == 2):
                        param = tokens[1]
                        if (param in list(Params.parameters.keys())):
                            ...
                            for p in list(Params.parameters.keys()):
                    ...
                    if (len(tokens) >= 3):
                        ...
                        if (param in list(Params.__dict__.keys())):
                            if (Params.parameters[param]["choices"]):
                                if (
                                        value in Params.parameters[param]["choices"]):
                                    ...
                                    self.logger.print_error(
                                        "Invalid value ({:s}) for field {:s}.".format(
                                            value, param))
                                    self.logger.print_info("Values for field are : {:s}.".format(
                                        ','.join(str(Params.parameters[param]["choices"]))))
                            else:
                                Params.__dict__[param] = value
                                new_value = Params.__dict__[param]
                                self.logger.print_success(
                    ...
                    if len(tokens) == 2:
                        ...
                        tmpdict = {}
                        for param in list(Params.parameters.keys()):
                            value = Params.__dict__[param]
                            tmpdict[param] = value
                ...
                self.logger.print_error(
                    "An exception as occured: {:s}".format(
                        e.message))
