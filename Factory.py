...
class Factory(object):
    ...
    def new_message(self, _args):
        ...
        for field_name, field_value in list(_args.__dict__.items()):
            # Validate the field given
            if (field_value is not None and field_name in list(
                    new_message.header.elements.keys())):
        ...
        for (code, group) in new_message.header.groups().items():
    ...
    def read_message(self, _bitstream):
        ...
        if (_bitstream is not None):
            # Creates a new VMF message object
            new_message = Message()

            # Iterate thru the parameters provided by the user to
            # create the message object with default values.
            for (field_name, field_obj) in new_message.header.fields().items():
            ...
            for (code, group) in new_message.header.groups().items():
                parent_group = group.parent_group
                if (parent_group is not None):
                    new_message.header.elements[parent_group].fields.append(
                        group)

            new_message.header.sort()
            root_grp = new_message.header.elements[CODE_GRP_HEADER]
            self.logger.print_debug(
                "Creating message from stream:\n{:s}".format(
                    _bitstream.hex))
    ...
    def read_message_rec(self, _element, _bitstream):
        ...
        if (isinstance(_element, Field)):
            ...
            # simply read the value.
                ...
                if _element.is_string:
                    ...
                    _element.value = _bitstream.read(
                        'uint:{:d}'.format(_element.size))

            self.logger.print_debug(
                "Processing field '{:s}': FPI={:d}, FRI={:d}, value={}".format(
        ...
        # if present, start processing all contained fields and subgroups.
            ...
            raise Exception(
                "Unknown/Unsupported object type: {:s}".format(type(_element)))
=======
class Factory(object):
