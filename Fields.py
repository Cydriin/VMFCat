...
class Field(HeaderElement):
    ...
    def __repr__(self):
        return "<Field: {:d}:{:s}:{:s}>".format(
            self.index, self.name, str(self.value))
    ...
    def get_value_from_dict(self, _key, _dict):
        for key, value in list(_dict.__dict__.items()):
    ...
    def get_bit_array(self):
        ...
        if (self.is_indicator):
            ...
            if (self.name == "Version" and self.enumerator):
                field_value = self.get_value_from_dict(
                    self.value, self.enumerator)
            ...
            if (self.pi == PRESENT):
                ...
                # if (self.pi == PRESENT or self.is_indicator):
                    b.append(self.format_str.format(field_value))
                elif (isinstance(field_value, str) or isinstance(field_value, str)):
                    sb = self.string_to_bitarray(field_value)
                    b.append(sb)
                else:
                    raise Exception(
                        "Unsupported type for field {:s}: {:s}".format(
                            self.name, type(field_value)))
            return b
    ...
    def string_to_bitarray(self, _string, _maxsize=448):
        ...
        if (len(b.bin) > _maxsize):
            raise Exception(
                "Size of bit array exceeds the maximum size allowed ({:d}).".format(_maxsize))
        return b
=======
# Contains common properties to VMF fields 
class Field(HeaderElement):
...
<<<<<<< HEAD
                # TODO: the year should only contain the last 2 digits, not the
                # entire
                date_obj = datetime.strptime(
                    date_items[0] + ' ' + date_items[1], format_str)
...
<<<<<<< HEAD
                raise Exception(
                    "Unknown datetime group format: {:s}.".format(_value))
    ...
    def get_bit_array(self):
        b = BitArray()
        dtgfields = list(self.fields.values())
        # self.fields.sort()
=======
...
>>>>>>> 933a3d67a02755010a5deb0093354e910846c0fe
    ...
    def __repr__(self):
<<<<<<< HEAD
        return "<Datetime Group Field: {:d}:{:s}:{:s}>".format(
            self.index, self.name, str(self.value))
